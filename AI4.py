...
class env:
	...
	def reset(self, match, n, numero, form, size, typ):  
		...
		self.state = get_state(self.mapp, self.bx, self.by, self.x, self.y, self.size, self.allies, self.enemies, self.team, self.loyalty)
	...
	def step(self, action):
		if action == 0:
			...
			if ris == 'BLOCKED':
				if self.y!= 0 and self.mappa_stato[self.y-1][self.x] == 100 and state[12] == 0:
...
def get_state(mapp, bx, by, x, y, size, allies, enemies, t, l):
	stato = np.zeros(13)
	if t != l:
		stato[12] = 1
	if y > by:
